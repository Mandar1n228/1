...
VIS_BLOCK_SCOPE = "block"
VIS_SERVICE_SCOPE = "service"
VIS_EXTERNAL_SCOPE = "external"
VIS_PING_SCOPE = "ping"
VIS_BLOCK_KEYS = ("services", "external", "ping")
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .layout.left-main { grid-template-columns: minmax(260px, 1fr) minmax(720px, 2fr); }
    .layout.left-ping { grid-template-columns: minmax(260px, 1fr) minmax(420px, 1.3fr); }
    .layout.left-only { grid-template-columns: minmax(260px, 1fr); }
...
<body>
  <div class="layout {{ layout_mode }}">
    <div class="left-column">
      ...
      <aside class="panel calendar-panel">
        ...
        <div class="calendar-header">
          <button id="prevMonth" class="calendar-nav-btn" type="button" aria-label="Попередній місяць">◀</button>
          <div id="calendarMonth" class="calendar-month">-</div>
          <button id="nextMonth" class="calendar-nav-btn" type="button" aria-label="Наступний місяць">▶</button>
        </div>
        <div class="calendar-weekdays" id="calendarWeekdays"></div>
        <div class="calendar-grid" id="calendarGrid"></div>
        <div class="next-control" id="nextControlInfo">Найближчий контроль: подій немає.</div>
      </aside>
    </div>

    <main class="card" {% if not show_main %}style="display:none;"{% endif %}>
    ...
    <aside class="panel" {% if not show_ping %}style="display:none;"{% endif %}>
  ...
  <div class="modal-backdrop" id="calendarModalBackdrop" role="dialog" aria-modal="true" aria-labelledby="calendarModalTitle">
    <div class="modal">
      <h3 id="calendarModalTitle">Контролі на <span id="calendarModalDateLabel">-</span></h3>
      <div class="field">
        <label for="controlName">Назва контролю</label>
        <input id="controlName" type="text" maxlength="120" placeholder="Напр. Перевірка документів" />
      </div>
      <div class="modal-actions">
        <button id="cancelControlEdit" type="button" class="modal-btn">Скасувати редагування</button>
        <button id="saveControl" type="button" class="modal-btn primary">Зберегти</button>
        <button id="closeCalendarModal" type="button" class="modal-btn">Закрити</button>
      </div>
  ...
  <div class="modal-backdrop" id="soundModalBackdrop" role="dialog" aria-modal="true" aria-labelledby="soundModalTitle">
    <div class="modal">
      ...
      <div id="adminSection" style="display:none; margin-top:12px; border-top:1px solid var(--panel-border); padding-top:10px;">
        ...
        <h4 style="margin:14px 0 8px;">Видимість блоків/кнопок для користувачів</h4>
        <div class="field">
          <label for="visibilityUserSelect">Користувач для налаштування</label>
          <select id="visibilityUserSelect"></select>
        </div>
        <div id="visibilitySettings" class="custom-list"></div>
        <div class="modal-actions" style="justify-content:flex-start;">
          <button id="saveVisibilityBtn" type="button" class="modal-btn primary">Зберегти видимість</button>
        </div>
      </div>
  ...
  <script>
    ...
    const visibilityCatalog = {{ visibility_catalog|tojson }};
    ...
    const visibilityUserSelect = document.getElementById("visibilityUserSelect");
    let selectedVisibilityUserId = null;
    ...
    const nextControlInfoEl = document.getElementById("nextControlInfo");
    const prevMonthBtn = document.getElementById("prevMonth");
    const nextMonthBtn = document.getElementById("nextMonth");

    const calendarModal = document.getElementById("calendarModalBackdrop");
    const calendarModalDateLabel = document.getElementById("calendarModalDateLabel");
    const controlNameInput = document.getElementById("controlName");
    const saveControlBtn = document.getElementById("saveControl");
    const closeCalendarModalBtn = document.getElementById("closeCalendarModal");
    const cancelControlEditBtn = document.getElementById("cancelControlEdit");
    ...
    function renderEventsForSelectedDate() {
      ...
      const list = controlEvents.filter((e) => e.date === selectedDateKey);

      if (!list.length) {
        const empty = document.createElement("div");
        empty.className = "custom-item-meta";
        empty.textContent = "На цю дату контролів немає.";
      ...
      list.forEach((eventItem) => {
        ...
        editBtn.addEventListener("click", () => {
          ...
          controlNameInput.value = eventItem.title;
          controlNameInput.focus();
        });
        ...
        deleteBtn.addEventListener("click", () => {
          ...
          if (editingEventId === eventItem.id) resetControlForm();
        });
    ...
    function updateNearestControl() {
      const today = new Date();
      today.setHours(0, 0, 0, 0);

      const upcoming = controlEvents
        .map((e) => ({ ...e, dateObj: parseDateKey(e.date) }))
        .filter((e) => e.dateObj >= today)
        .sort((a, b) => a.dateObj - b.dateObj || a.title.localeCompare(b.title, "uk"));

      if (!upcoming.length) {
        nextControlInfoEl.textContent = "Найближчий контроль: подій немає.";
        return;
      }

      const next = upcoming[0];
      nextControlInfoEl.innerHTML = `Найближчий контроль: <strong>${formatDateUA(next.dateObj)} — ${next.title}</strong>`;
    }
    ...
    function renderCalendar() {
      ...
      const cellsCount = Math.ceil((mondayBased + daysInMonth) / 7) * 7;
      const startDate = new Date(year, month, 1 - mondayBased);
      ...
      for (let i = 0; i < cellsCount; i++) {
        ...
        if (date.getMonth() !== month) btn.classList.add("muted");
        if (dateKey === todayKey) btn.classList.add("today");
        if (controlEvents.some((e) => e.date === dateKey)) btn.classList.add("has-event");

        btn.addEventListener("click", () => openCalendarModal(dateKey));
    ...
    async function renderUsersList() {
      ...
      usersList.innerHTML = "";
      if (visibilityUserSelect) {
        visibilityUserSelect.innerHTML = "";
        users
          .filter((u) => !u.is_admin)
          .forEach((u) => {
            ...
            visibilityUserSelect.appendChild(opt);
          });
        if (!selectedVisibilityUserId && visibilityUserSelect.options.length > 0) {
          selectedVisibilityUserId = visibilityUserSelect.options[0].value;
        }
        if (selectedVisibilityUserId) {
          visibilityUserSelect.value = String(selectedVisibilityUserId);
        }
      }
      users.forEach((user) => {
        const item = document.createElement("div");
      ...
      if (selectedVisibilityUserId) {
        await renderVisibilitySettings(selectedVisibilityUserId);
      } else if (visibilitySettings) {
        visibilitySettings.innerHTML = "<div class='custom-item'>Немає користувачів для налаштування.</div>";
      }
    }

    async function renderVisibilitySettings(userId) {
      if (!authMe?.is_admin || !visibilitySettings || !userId) return;
      selectedVisibilityUserId = String(userId);
      const r = await fetch(`/api/ui-visibility/${selectedVisibilityUserId}`, { cache: "no-store" });
      if (!r.ok) {
        visibilitySettings.innerHTML = "<div class='custom-item'>Не вдалося завантажити налаштування.</div>";
        return;
      }
      const state = await r.json();
      visibilitySettings.innerHTML = "";
      const groups = [
        { title: "Блоки", id: "blocks", scope: "block", items: visibilityCatalog.blocks || [] },
        { title: "Сервіси переходу", id: "services", scope: "service", items: visibilityCatalog.services || [] },
        { title: "Зовнішні сервіси", id: "external", scope: "external", items: visibilityCatalog.external || [] },
        { title: "Ping цілі", id: "ping", scope: "ping", items: visibilityCatalog.ping || [] },
      ];
      groups.forEach((group) => {
        ...
        group.items.forEach((item) => {
          ...
          input.type = "checkbox";
          input.checked = state[group.scope]?.[item.key] !== false;
          input.dataset.scope = group.id;
          input.dataset.key = item.key;
          line.appendChild(input);
          line.append(` ${item.label}`);
          box.appendChild(line);
        });
        visibilitySettings.appendChild(box);
      });
    }
    ...
    if (saveVisibilityBtn) {
      saveVisibilityBtn.addEventListener("click", async () => {
        if (!authMe?.is_admin) return;
        if (!selectedVisibilityUserId) {
          alert("Оберіть користувача для налаштування.");
          return;
        }
        const payload = { blocks: {}, services: {}, external: {}, ping: {} };
        visibilitySettings.querySelectorAll("input[type='checkbox'][data-scope][data-key]").forEach((node) => {
          const scope = node.dataset.scope;
          const key = node.dataset.key;
          if (!payload[scope]) return;
          payload[scope][key] = node.checked;
        });
        const r = await fetch(`/api/ui-visibility/${selectedVisibilityUserId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
        if (!r.ok) {
          alert("Не вдалося зберегти налаштування видимості.");
          return;
        }
        alert("Налаштування видимості збережено. Оновіть сторінку.");
      });
    }
    if (visibilityUserSelect) {
      visibilityUserSelect.addEventListener("change", () => {
        selectedVisibilityUserId = visibilityUserSelect.value || null;
        if (selectedVisibilityUserId) {
          renderVisibilitySettings(selectedVisibilityUserId);
        }
      });
    }
...
def _service_key(item: Tuple[str, str, int]) -> str:
    return str(item[2])


def _external_key(item: Tuple[str, str, str]) -> str:
    return item[0]


def _ping_key(item: Tuple[str, str]) -> str:
    return item[1]


def _visibility_default_records() -> List[Tuple[str, str, int]]:
    defaults: List[Tuple[str, str, int]] = []
    defaults.extend((VIS_BLOCK_SCOPE, key, 1) for key in VIS_BLOCK_KEYS)
    defaults.extend((VIS_SERVICE_SCOPE, _service_key(item), 1) for item in TARGET_APPS)
    defaults.extend((VIS_EXTERNAL_SCOPE, _external_key(item), 1) for item in EXTERNAL_LINKS)
    defaults.extend((VIS_PING_SCOPE, _ping_key(item), 1) for item in PING_TARGETS)
    return defaults


def _ensure_user_visibility_defaults(conn: sqlite3.Connection, user_id: int) -> None:
    defaults = [(user_id, scope, item_key, enabled) for scope, item_key, enabled in _visibility_default_records()]
    conn.executemany(
        """
        INSERT OR IGNORE INTO user_ui_visibility (user_id, scope, item_key, enabled)
        VALUES (?, ?, ?, ?)
        """,
        defaults,
    )
...
def _load_visibility(conn: sqlite3.Connection, user_id: int) -> Dict[str, Dict[str, bool]]:
    _ensure_user_visibility_defaults(conn, user_id)
    rows = conn.execute(
        "SELECT scope, item_key, enabled FROM user_ui_visibility WHERE user_id = ?",
        (user_id,),
    ).fetchall()
    ...
    for row in rows:
        ...
        if scope not in data:
            continue
        data[scope][row["item_key"]] = bool(row["enabled"])
    return data
...
def _init_auth_storage() -> None:
    ...
    with _db_conn() as conn:
        ...
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS user_ui_visibility (
        ...
        for row in users:
            _ensure_user_visibility_defaults(conn, int(row["id"]))
        conn.commit()
...
def index():
    ...
    with _db_conn() as conn:
        visibility = _load_visibility(conn, int(user["id"]))

    block_visibility = visibility.get(VIS_BLOCK_SCOPE, {})
    is_admin = bool(user and user["is_admin"])

    services_enabled = bool(block_visibility.get("services", True))
    external_enabled = bool(block_visibility.get("external", True))
    ping_enabled = bool(block_visibility.get("ping", True))

    service_items = []
    for svc in TARGET_APPS:
        key = _service_key(svc)
        if not is_admin and (not services_enabled or not visibility[VIS_SERVICE_SCOPE].get(key, True)):
            continue
        name, _, port = svc
        service_items.append({"name": name, "url": f"http://{gateway_host}:{port}", "key": key})

    external_items = []
    for ext in EXTERNAL_LINKS:
        key = _external_key(ext)
        if not is_admin and (not external_enabled or not visibility[VIS_EXTERNAL_SCOPE].get(key, True)):
            continue
        name, url, icon = ext
        external_items.append({"name": name, "url": url, "icon": _resolve_icon_url(name, icon), "key": key})

    ping_targets = []
    for target in PING_TARGETS:
        key = _ping_key(target)
        if not is_admin and (not ping_enabled or not visibility[VIS_PING_SCOPE].get(key, True)):
            continue
        name, host = target
        ping_targets.append({"name": name, "host": host, "key": key})

    show_main = bool(service_items or external_items) or is_admin
    show_ping = bool(ping_targets) or is_admin
    layout_mode = "full"
    if not show_main and not show_ping:
        layout_mode = "left-only"
    elif not show_main:
        layout_mode = "left-ping"
    elif not show_ping:
        layout_mode = "left-main"

    visibility_catalog = {
        "blocks": [
            {"key": "services", "label": "Оберіть сервіс для переходу"},
            {"key": "external", "label": "Зовнішні сервіси"},
            {"key": "ping", "label": "Ping у реальному часі"},
        ],
        "services": [{"key": _service_key(item), "label": item[0]} for item in TARGET_APPS],
        "external": [{"key": _external_key(item), "label": item[0]} for item in EXTERNAL_LINKS],
        "ping": [{"key": _ping_key(item), "label": f"{item[0]} ({item[1]})"} for item in PING_TARGETS],
    }
    ...
    return render_template_string(
        ...
        visibility_catalog=visibility_catalog,
...
@app.route("/api/ui-visibility/<int:user_id>", methods=["GET", "PUT"])
@_require_login
def ui_visibility_api(user_id: int):
    actor = _current_user()
    if actor is None or not actor["is_admin"]:
        return jsonify({"error": "forbidden"}), 403
    with _db_conn() as conn:
    ...
    if bool(target["is_admin"]):
        return jsonify({"error": "admin visibility is not configurable"}), 400

    if request.method == "GET":
        with _db_conn() as conn:
            visibility = _load_visibility(conn, user_id)
    ...
    def _to_pairs(scope: str, values: Dict[str, object], allowed: List[str]) -> List[Tuple[int, str, str]]:
        result: List[Tuple[int, str, str]] = []
        allowed_set = set(allowed)
        for key, enabled in values.items():
            if key not in allowed_set:
                continue
            result.append((1 if bool(enabled) else 0, scope, key))
        return result

    updates: List[Tuple[int, str, str]] = []
    updates += _to_pairs(VIS_BLOCK_SCOPE, blocks, list(VIS_BLOCK_KEYS))
    updates += _to_pairs(VIS_SERVICE_SCOPE, services, [_service_key(item) for item in TARGET_APPS])
    updates += _to_pairs(VIS_EXTERNAL_SCOPE, external, [_external_key(item) for item in EXTERNAL_LINKS])
    updates += _to_pairs(VIS_PING_SCOPE, ping, [_ping_key(item) for item in PING_TARGETS])

    with _db_conn() as conn:
        _ensure_user_visibility_defaults(conn, user_id)
        if updates:
            conn.executemany(
                "UPDATE user_ui_visibility SET enabled = ? WHERE user_id = ? AND scope = ? AND item_key = ?",
                [(enabled, user_id, scope, item_key) for enabled, scope, item_key in updates],
            )
        conn.commit()
